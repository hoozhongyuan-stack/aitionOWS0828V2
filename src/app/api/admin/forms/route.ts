...
export async function PUT(req: Request) {
  ...
  void logAdmin({ adminId: admin?.id ?? null, adminName: admin?.name ?? "?", action: "forms.put", ip: getClientIp(req) });
  if ("error" in guard) return guard.error;
...
export async function DELETE(req: Request) {
  const guard = await requirePerm("content");
  const admin = "admin" in guard ? guard.admin : null;
  void logAdmin({ adminId: admin?.id ?? null, adminName: admin?.name ?? "?", action: "forms.delete", ip: getClientIp(req) });
  if ("error" in guard) return guard.error;
