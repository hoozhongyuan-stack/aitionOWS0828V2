...
import { setNavItemVisible, deleteNavItem } from "@/server/content";

/**
 * 导航批量操作(V4.4.0):POST /api/admin/nav/batch
 * 删除沿用 deleteNavItem 的既有行为(含其子项一并清理),显隐只改 visible 单字段。
 */
...
export async function POST(req: Request) {
  ...
  const outcome = await runBatch(norm.ids, async (id) => {
    ...
    await setNavItemVisible(id, action === "show");
  });

  void logAdmin({
    adminId: admin?.id ?? null,
    adminName: admin?.name ?? "?",
    action: `nav.batch.${action}`,
