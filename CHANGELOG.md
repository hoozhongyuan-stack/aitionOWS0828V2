...
- **批量操作**:内容/栏目/导航/注册用户/互动审核/商品**六个列表**支持勾选与批量操作。通用执行器**逐项执行并回报跳过原因**(不做"全成功或全失败");护栏含单次 100 条上限、删除二次确认、翻页清空选择、一次批量一条审计日志、技术错误清洗(不外泄 Prisma 细节);**注册用户不开放批量删除**(连带订单/评论/收藏/投稿,删除保持单个)、**投稿不开放批量删除**(属内容管理职责)
